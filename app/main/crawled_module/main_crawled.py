...
class MainCrawledProcess(object):
    ...
    def grading_first_column(input_info):
        ...
        crawled_logger.debug(msg="开始采集一级栏目{input_info}".format(input_info=input_info))
        try:
            # 1. 识别当前网站的反爬虫协议
            ...
            if my_request_module.main_process(url=input_info["url"]):
                ...
                if response_html:
                    crawled_logger.debug(msg="渲染网站首页成功{input_info}".format(input_info=input_info))
                    my_soup = bs4.BeautifulSoup(response_html, features="lxml")
                    a_list = my_soup.find_all("a")
                    # 3. 加载图片信息
                    crawled_logger.debug(msg="开始处理图像栏目{input_info}".format(input_info=input_info))
                    ...
                    # 4. 过滤a标签
                    crawled_logger.debug(msg="开始处理a标签{input_info}".format(input_info=input_info))
                    ...
                    my_attach_handle_module.column_filter_main(a_list=a_list,
                                                               ...
                                                               parent_id=None)
                    crawled_logger.debug(msg="成功采集一级栏目信息")
                    logging.shutdown()
                    return True
                else:
                    crawled_logger.debug(msg="渲染网站首页失败{input_info}".format(input_info=input_info))
            else:
                crawled_logger.debug(msg="网络有爬虫协议或网络未连通{input_info}".format(input_info=input_info))
            logging.shutdown()
    ...
    def grading_second_column(input_info):
        ...
        crawled_log_filename = crawled_logging.get_log_filename(dir_path=crawled_dir_path)
        crawled_logger = crawled_logging.log(log_filename=crawled_log_filename, level="DEBUG")
        crawled_logger.debug(msg="开始采集二级栏目{input_info}".format(input_info=input_info))
        try:
            # 1. 获取所有待采集的一级栏目列表
            my_attach_handle_module = attach_handle_module.AttachHandle(website=input_info["url"],
                                                                        website_id=input_info["id"])
            ...
            if first_column_list:
                for i in first_column_list:
                    crawled_logger.debug(msg="一级栏目信息{i}".format(i=i))
                    # 2. 获取所有a标签列表
                    my_selenium_module = selenium_module.SeleniumModule()
                    response_html = my_selenium_module.loading_html(input_url=i["column_url"])
                    my_selenium_module.quit_browser()
                    if response_html:
                        crawled_logger.debug(msg="加载一级栏目页面成功{input_info}".format(input_info=input_info))
                        my_soup = bs4.BeautifulSoup(response_html, features="lxml")
                        a_list = my_soup.find_all("a")
                        # 3. 加载图片信息
                        crawled_logger.debug(msg="开始处理图片{input_info}".format(input_info=input_info))
                        my_loading_img_module = loading_img_module.LoadingImg(website=input_info["url"],
                                                                              website_id=input_info["id"],
                                                                              a_list=a_list,
                        ...
                        my_attach_handle_module.column_filter_main(a_list=a_list, img_info_list=img_info_list,
                                                                   parent_id=i[0])
                        crawled_logger.debug(msg="成功采集二级栏目信息{i}".format(i=i))
                    else:
                        crawled_logger.debug(msg="加载一级栏目页面失败{i}".format(i=i))
                logging.shutdown()
                return True
            else:
                crawled_logger.debug(msg="获取一级栏目列表失败{input_info}".format(input_info=input_info))
                logging.shutdown()
        except Exception as e:
            crawled_logger.error(msg=e)
            logging.shutdown()
    ...
    def grading_third_column(input_info):
        ...
        crawled_log_filename = crawled_logging.get_log_filename(dir_path=crawled_dir_path)
        crawled_logger = crawled_logging.log(log_filename=crawled_log_filename, level="DEBUG")
        crawled_logger.debug(msg="开始采集三级栏目{input_info}".format(input_info=input_info))
        try:
            # 1. 获取所有待采集的二级栏目列表
            my_attach_handle_module = attach_handle_module.AttachHandle(website=input_info["url"],
                                                                        website_id=input_info["id"])
            ...
            if second_column_list:
                for i in second_column_list:
                    crawled_logger.debug(msg="待分析的二级栏目信息{i}".format(i=i))
                    # 2. 获取所有a标签列表
                    my_selenium_module = selenium_module.SeleniumModule()
                    response_html = my_selenium_module.loading_html(input_url=i["second_level_column"])
                    my_selenium_module.quit_browser()
                    if response_html:
                        crawled_logger.debug(msg="加载二级栏目页面{input_info}".format(input_info=input_info))
                        my_soup = bs4.BeautifulSoup(response_html, features="lxml")
                        a_list = my_soup.find_all("a")
                        # 3. 加载图片信息
                        crawled_logger.debug(msg="开始处理图片{input_info}".format(input_info=input_info))
                        ...
                        img_info_list = my_loading_img_module.loading_img_main()
                        # 5. 过滤a标签
                        crawled_logger.debug(msg="开始过滤a标签{input_info}".format(input_info=input_info))
                        my_attach_handle_module = attach_handle_module.AttachHandle(website=input_info["url"],
                                                                                    website_id=input_info["id"])
                        my_attach_handle_module.column_filter_main(a_list=a_list,
                                                                   img_info_list=img_info_list,
                                                                   parent_id=i[0])
                        crawled_logger.debug(msg="成功采集三级栏目信息{i}".format(i=i))

                    else:
                        crawled_logger.debug(msg="加载二级栏目页面失败{input_info}".format(input_info=input_info))
                logging.shutdown()
                return True
            else:
                crawled_logger.debug(msg="获取二级栏目列表失败{input_info}".format(input_info=input_info))
                logging.shutdown()
        except Exception as e:
            crawled_logger.error(msg=e)
            logging.shutdown()
    ...
    def grading_column(self, website_id):
        ...
        try:
            ...
            if website_info:
                crawled_logger.debug(msg="开始采集栏目{website_id}".format(website_id=website_id))
                my_database_module = database_module.DatabaseModule()
                ...
                if self.grading_first_column(input_info=website_info):
                    if self.grading_second_column(input_info=website_info):
                        if self.grading_third_column(input_info=website_info):
                            ...
                            my_database_module.update_data(sql_sentence=sql_sentence, field_list=field_list)
                            crawled_logger.debug(msg="采集三级栏目失败{website_id}".format(website_id=website_id))
                    else:
                        my_database_module = database_module.DatabaseModule()
                        sql_sentence = "UPDATE crawled_website_info SET execution_status=4 WHERE id=%s"
                        my_database_module.update_data(sql_sentence=sql_sentence, field_list=field_list)
                        crawled_logger.debug(msg="采集二级栏目失败{website_id}".format(website_id=website_id))
                else:
                    my_database_module = database_module.DatabaseModule()
                    sql_sentence = "UPDATE crawled_website_info SET execution_status=4 WHERE id=%s"
                    my_database_module.update_data(sql_sentence=sql_sentence, field_list=field_list)
                    crawled_logger.debug(msg="采集一级栏目失败{website_id}".format(website_id=website_id))
            else:
                crawled_logger.debug(msg="获取网站信息失败{website_id}".format(website_id=website_id))
                my_database_module = database_module.DatabaseModule()
                sql_sentence = "UPDATE crawled_website_info SET execution_status=4 WHERE id=%s"
                my_database_module.update_data(sql_sentence=sql_sentence, field_list=field_list)
            logging.shutdown()
    ...
    def crawled_article(self, website_id):
        ...
        try:
            ...
            if my_attach_handle_module.crawled_article_main():
                crawled_logger.debug(msg="根据栏目爬取文章成功{website_info}".format(website_info=website_info))
            else:
                crawled_logger.debug(msg="根据栏目爬取文章失败{website_info}".format(website_info=website_info))
            logging.shutdown()
        except Exception as e:
            crawled_logger.error(msg=e)
            logging.shutdown()
    ...
    def analyze_article(self, website_id):
        ...
        crawled_logger = crawled_logging.log(log_filename=crawled_log_filename, level="DEBUG")
        try:
            crawled_logger.debug(msg="开始分析文章{website_id}".format(website_id=website_id))
            website_info = self.get_url(input_id=website_id)
            my_database_module = database_module.DatabaseModule()
            ...
            if my_data:
                for i in my_data:
                    ...
                    crawled_logger.debug(msg="分析文章，文章id为{article_id}".format(article_id=article_info["id"]))
                ...
                crawled_logger.debug(msg="未获取到文章列表{website_id}".format(website_id=website_id))
            logging.shutdown()
        except Exception as e:
            crawled_logger.error(msg=e)
            logging.shutdown()
    ...
    def one_crawled_task(self, website_id):
        ...
        crawled_log_filename = crawled_logging.get_log_filename(dir_path=crawled_dir_path)
        crawled_logger = crawled_logging.log(log_filename=crawled_log_filename, level="DEBUG")
        crawled_logger.debug(msg="开始爬虫任务{website_id}".format(website_id=website_id))
        field_list = []
        my_crawled_logging_module = crawled_logging_module.CrawledLoggingModule()
...
if __name__ == '__main__':
    ...
    main_crawled_process.multiprocessing_grading(website_id=31)
