...
class CrawledLogging(object):
    ...
    def debug_log_main(self, message):
        crawled_dir_path = self.make_log_dir(log_dir_name="crawled_log_debug")
        ...
        for i in crawled_logger.handlers:
            crawled_logger.removeHandler(i)
        logging.shutdown()

    def error_log_main(self, message):
        crawled_dir_path = self.make_log_dir(log_dir_name="crawled_log_error")
        crawled_log_filename = self.get_log_filename(dir_path=crawled_dir_path)
        crawled_logger = self.log(log_filename=crawled_log_filename, level="ERROR")
        crawled_logger.error(msg=message)
        my_hdlr = logging.FileHandler(crawled_log_filename)
        crawled_logger.removeHandler(my_hdlr)
        logging.shutdown()
...
if __name__ == '__main__':
    test_crawled_logging = CrawledLogging()
    test_crawled_logging.error_log_main(message="123")
    test_crawled_logging.debug_log_main(message="456")
